...
**Key inputs:** `q` (name fragment) and `datasetKey` (to scope to a specific checklist), both validated whenever present since `/species/search` answers a blank one with the unfiltered backbone result; `rank`, `kingdom`, `family`, `genus`, `isExtinct`, `limit` (default 20, max 1000), `offset`. See decision 16, which also records that `kingdom`, `family`, and `genus` are inert upstream.
...
The rule covers a blank value even where the upstream answer is not the unfiltered scope, because the alternative is a boundary no caller can plan around. `/dataset/search?q=` returns all 123,527 indexed datasets while `?q=%20%20` returns none; `/species/search` splits the same way, 46,623,747 against 0. One space between the whole index and an empty page is not a distinction to leave running, and neither side of it is the search that was asked for. `/organization` does not split — it drops `q` in either form and answers with all 3,561 organizations against 460 for `museum`.
...
`kingdom`, `family`, and `genus` on `gbif_search_species` are deliberately outside the rule, because there is no filter there for a blank value to drop. `/species/search` implements none of the three: each answers identically to a parameter name invented for the probe — 46,623,747 names with `family=Paridae`, with `genus=Quercus`, with `kingdom=Plantae`, and with `bogusParam=zzz`, against `rank=FAMILY`'s 558,588 — so no value of theirs narrows anything and a blank one is neither better nor worse than a real one. Guarding them would advertise a validation on fields that never filter, which is a worse claim than the one it removes. The three are left exactly as they are and the inertness is tracked as its own defect.
...
| Species search | `GET /species/search` | `q`, `rank`, `datasetKey`, `limit`, `offset`; a blank `q` returns the whole index while a whitespace-only one returns nothing. `kingdom`, `family`, and `genus` are **not implemented** — each answers identically to an invented parameter name, at every value |
