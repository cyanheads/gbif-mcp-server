...
| `gbif_search_datasets` | Search GBIF datasets by keyword, type, country, or hosting organization. Returns dataset title, description, license, record count, and DOI. | `q`, `type`, `publishingCountry`, `hostingOrg`, `limit`, `offset` | `readOnlyHint: true` |
...
**Key inputs:** `q` (free text), `type` (OCCURRENCE | CHECKLIST | METADATA | SAMPLING_EVENT), `publishingCountry` (`^[A-Z]{2}$` — `/dataset/search` matches the verbatim stored code, so `gb` and `GBR` return zero datasets rather than an error; see decision 14), `hostingOrg` (organization UUID), `limit` (default 20), `offset`
...
The pattern is applied per endpoint, because the endpoints do not agree. `/occurrence/search` parses the code and then matches the verbatim string, so a form that parses but is not stored answers 200 with zero records — `gb`, `Us`, `uS`, `USA`, and `gb ` all return 0 against `taxonKey=212` + `occurrenceStatus=PRESENT`, where `GB` returns 60,290,950. `/dataset/search` behaves the same way one layer up: `gb`, `Gb`, `gB`, `gbr`, `GBR`, and `USA` each return 0 datasets where `GB` returns 2,416 and `US` returns 3,246. `/organization`, behind `gbif_search_publishers`, resolves the parsed country instead: `gb`, `gbr`, `GBR`, and `Gb` each return the same 223 organizations as `GB`, and `USA` the same 499 as `US`. There is no silent zero to close there, so the same pattern would only reject values that currently answer correctly — a regression dressed as consistency. `gbif_search_publishers` keeps a plain string, and its description names the divergence so the rule the other three tools follow is not over-generalized onto it.

Only `publishingCountry` moves on `gbif_search_datasets`. Its other filters are already guarded or have nothing to guard against: `type` is an enum, `hostingOrg` is UUID-checked in the handler and fails as `invalid_filter`, and `q` is free text with no vocabulary behind it — the same shape as `stateProvince`, and answered the same way, by the empty-result notice. `/dataset/search` also accepts `hostingCountry`, which carries the identical hole (`hostingCountry=gb` returns 0 against `GB`'s 1,240), but it is not exposed by this server and is not added here.
...
| Dataset search | `GET /dataset/search` | q, type, publishingCountry — matches the verbatim stored code, so only the uppercase alpha-2 form resolves |
| Dataset get | `GET /dataset/{uuid}` | |
| Publisher search | `GET /organization` | q, country — resolves the parsed country, so alpha-2 and alpha-3 forms both match, case-insensitively |
